...
function Changepass () {
    ...
    //  if (data.password == )
    if (values.oldPassword == data[0].password && isValidPassword(values.newPassword) && values.newPassword == data[0].confirmNewPassword ){
         patchPass({
            id: data[0].id,
    ...
    return (
      ...
      >
        ...
        >
          <Input.Password style={{ width: '450px' }} onBlur={handleOnblur_2} onChange={handleChange_2} />
         
        </Form.Item>
        <span className={data_2 ? 'handleNotice' : 'span'} id="notice">Vui lòng nhập tối thiểu 8 kí tự, một chữ thường, 1 chữ hoa và 1 số </span>
        <Form.Item
