...
const handleXoa = () => {     // chức năng xóa khi select
    ...
    const dataAfterDel =  dataFilter.reduce ((origin, item) => {   // origin là mảng đầu tiên
...
const handleDelitem = (e) => {   // xóa từng item
    deleteProduct(e.id);
    setReload(!reload);
}
const handleXem = (values) => {    // xem chi tiết từng item
    navigate("/admin/product/" + values.id )
}

const handleShow = () => {   // thêm sản phẩm
    setShow(true);
    
}

const handleAddProduct = (productData) => {
   const transformArray = productData.images.split(", ");
...
setReload(!reload);
  ...
  const columns = [
      ...
      render: (price, record) => (
        <span>
         {new Intl.NumberFormat("vi-VN", {
            style: "currency",
            currency: "VND",
          }).format(`${record.discountPercentage > 0
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space size="middle">
              <Tooltip title="Xem chi tiết" color='#085820' key='1'>  <Button type="primary" icon={<img width="20" height="20" src="https://img.icons8.com/external-gradak-royyan-wijaya/24/FFFFFF/external-eyes-gradak-interface-solidarity-gradak-royyan-wijaya.png" alt="external-eyes-gradak-interface-solidarity-gradak-royyan-wijaya"/>} className="productlist--delitem" onClick={() => handleXem(record)}></Button>
</Tooltip>
          <Tooltip title="Xóa" color='#085820' key='2'>  <Button type="primary" icon={<img width="19" height="19" src="https://img.icons8.com/ios-filled/50/FFFFFF/trash.png" alt="trash"/>} className="productlist--delitem" onClick={() => handleDelitem(record)}></Button>
</Tooltip>
  ...
  return (
    ...
    <Button type="primary" onClick={handleShow} danger>
       Thêm sản phẩm
    </Button>
         <Table
         bordered
    ...
    <AddProductModal reload={reload} open={show} onCancel={() => setShow(false)} onAddProduct={handleAddProduct} />
   </>
